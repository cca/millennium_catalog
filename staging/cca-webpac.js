...
var bib_title = "";
	...
	var searchterms = document.getElementsByName('searcharg')[0];
		...
		{
			document.title = document.title + " :: '" + searchterms.value + "' search results" }
		} 
catch(e)
  {
 alert ( e ); 
...
var otherlib_title = ""; 
	...
	else if (otherlib_title.length > 0)
	{ 
			...
			for(var j = 0; j < trs.length; j++) {
				if (trs[j].className=='ftlistHeader') { // check if there is any ISBN in the bibliographic record
					...
					for (var i=0; i < ths.length; i++) {
						if (ths[i].innerHTML=="Featured at CCA Libraries (4 entries)") {
							var ftlist = "Featured at CCA Libraries<br /><a href=\"http:\/\/feedburner.google.com\/fb\/a\/mailverify?uri=cca\/newbooks&amp;loc=en_US\" style='font-size: 12px;'>+ Sign-up for new titles monthly emails +<\/a>";
