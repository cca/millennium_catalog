...
function stripeBrowseTables() {
	...
	if (!yours[0]) {
		...
		for (var i = 0; i < myTR.length; i++) {
			if (myTR[i].className == "browseEntry odd" || myTR[i].className ==
				"browseEntry marked odd") {
		...
		if (document.attachEvent) {
			if (yours[0].nextSibling.className == "browseEntry" || yours[0].nextSibling
				.className == "browseEntry marked") {
				stripeBrowse();
				for (i = 0; i < tds.length; i++) {
					addClass(tds[i], "outline");
					if (i == 0) {
						addClass(tds[i], "leftCap");
					}
					if (i == (tds.length - 1)) {
						addClass(tds[i], "rightCap");
					}
				}
			} else {
				stripeBrowse();
						...
						if (spaner[0]) {
							if (spaner[0].getAttribute("ROWSPAN")) {
								...
								for (j = 0; j < spaner.length; j++) {
									...
									if (j == 0) {
										addClass(spaner[j], "leftCap");
									}
									if (j == (spaner.length - 1)) {
										addClass(spaner[j], "rightTopCap");
									}
								}
								for (k = 1; k < num; k++) {
									...
									if (k == (num - 1)) {
										for (l = 0; l < subs.length; l++) {
											...
											if (l == (subs.length - 1)) {
												addClass(subs[l], "rightBotCap");
											}
										}
									} else {
										for (l = 0; l < subs.length; l++) {
											if (l == (subs.length - 1)) {
												addClass(subs[l], "rightSide");
											}
										}
									}
								}
			...
			if (yours[0].nextSibling.nextSibling.className == "browseEntry" || yours[0]
				.nextSibling.nextSibling.className == "browseEntry marked") {
				stripeBrowse();
				for (i = 0; i < tds.length; i++) {
					addClass(tds[i], "outline");
					if (i == 0) {
